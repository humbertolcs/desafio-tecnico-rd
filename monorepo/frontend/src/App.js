...
function App() {
  ...
  // Rola a tela para a lista de recomendações quando ela é exibida
  useEffect(() => {
    if (recommendationListRef.current  && recommendations.length > 0) {
      recommendationListRef.current.scrollIntoView({ behavior: 'smooth' });
    }
  }, [recommendations]);
